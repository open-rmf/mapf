...
fn gather_points_of_interest<G: Grid>(
    ...
    for interest in points_of_interest {
        if visibility.grid().is_square_occupied(
    ...
    triangular_for(valid.iter(), |interest_i, interest_j| {
        ...
        if visibility.grid().is_sweep_occupied(
            interest_i.to_center_point(cell_size),
            interest_j.to_center_point(cell_size),
            2.0 * visibility.agent_radius(),
        ).is_none() {
            connections.entry(**interest_i).or_default().insert(*interest_j);
            connections.entry(*interest_j).or_default().insert(**interest_i);
        }
    });
...
impl<G: Grid> VisibilityGraph<G> {
    pub fn new(visibility: Arc<Visibility<G>>, points_of_interest: impl Iterator<Item=Cell>) -> Self {
        let (points_of_interest, visibility_of_interest) = gather_points_of_interest(visibility.as_ref(), points_of_interest);
        Self{visibility, visibility_of_interest, points_of_interest}
    }
}
...
impl<G: Grid> Graph for VisibilityGraph<G> {
    ...
    fn edges_from_vertex<'a>(&'a self, from_cell: Self::Key) -> Self::EdgeIter<'a> {
        ...
        .chain({
            ...
            .chain(
                [interest].into_iter().filter(|x| x.is_none())
                .flat_map(move |_| {
                    self.points_of_interest.iter()
                    .filter(move |poi| {
                        let to_p = poi.to_center_point(self.visibility.grid().cell_size());
                        let from_p = from_cell.to_center_point(self.visibility.grid().cell_size());
                        self.visibility.grid().is_sweep_occupied(
                            from_p,
                            to_p,
                            2.0 * self.visibility.agent_radius(),
                        ).is_none()
