...
pub trait InitAimless<Start>: Expander {
    type InitAimlessError: StdError;

    type InitialAimlessNodes<'a>: IntoIterator<
        Item = Result<Arc<Self::Node>, Self::InitAimlessError>,
        IntoIter: 'a,
    >
    where
        Self: 'a,
    ...
    fn aimless_start<'a>(&'a self, start: &'a Start) -> Self::InitialAimlessNodes<'a>;
}

pub trait Aimless: Expander {
    type AimlessError: StdError;
...
pub trait InitTargeted<Start, G: Goal<Self::Node>>: Expander {
    ...
    type InitTargetedError: StdError;

    /// An initial set of nodes, produced from a Start object
    type InitialTargetedNodes<'a>: IntoIterator<
        Item = Result<Arc<Self::Node>, Self::InitTargetedError>,
        IntoIter: 'a,
    >
    where
        Self: 'a,
    ...
    fn start<'a>(&'a self, start: &'a Start, goal: &'a G) -> Self::InitialTargetedNodes<'a>;
}

pub trait Targeted<G: Goal<Self::Node>>: Expander {
    /// The type of error that can occur during expansion.
    type TargetedError: StdError;
