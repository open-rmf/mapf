...
impl<E: Targeted<G>, C: Targeted<G, Node = E::Node>, G: Goal<E::Node>> Targeted<G> for Chain<E, C> {
    ...
    fn expand<'a>(
        ...
        self.base
            .expand(parent, goal)
            .into_iter()
            .map(|r| r.map_err(ChainErr::Base))
            .chain(
                self.chain_with
                    .expand(parent, goal)
                    .into_iter()
                    .map(|r| r.map_err(ChainErr::Next)),
            )
    }
}
...
pub trait Chainable {
    type Base: Expander;
    fn chain<C: Expander<Node = NodeOf<Self::Base>>>(self, chain_with: C) -> Chain<Self::Base, C>;
...
impl<E: Expander> Chainable for E {
    type Base = E;
    fn chain<C: Expander<Node = E::Node>>(self, chain_with: C) -> Chain<Self, C> {
        Chain {
            base: self,
            chain_with,
        }
