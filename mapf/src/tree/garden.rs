...
use crate::{
    expander::{
        Aimless, AimlessErrorOf, BidirSolvable, BidirSolveErrorOf, Closable, CostOf, Expander,
        InitAimless, InitAimlessErrorOf, NodeOf, ReversalErrorOf, ReverseNodeOf, ReverseOf,
        Reversible, SolutionOf, Solvable,
    },
    node::{self, ClosedSet, KeyOf, Keyed, KeyedSet, PartialKeyed, Weighted},
    util::Minimum,
};
use std::{
    cell::RefCell,
...
type SolutionCache<E> =
    MutexRefCell<HashMap<(KeyOf<NodeOf<E>>, KeyOf<NodeOf<E>>), Option<SolutionOf<E>>>>;
type ConnectionMap<E> =
    HashMap<KeyOf<NodeOf<E>>, (Option<Arc<NodeOf<E>>>, Option<Arc<NodeOf<ReverseOf<E>>>>)>;

fn new_tree_entry<E: Aimless<Node: Weighted> + Closable>(
    tree: Tree<E>,
) -> Arc<MutexRefCell<Tree<E>>> {
    ...
    NodeOf<E>: Weighted + PartialKeyed,
{
    ...
    pub fn solve<S>(
        ...
        from: &S,
        to: &KeyOf<NodeOf<E>>,
    ) -> Result<Option<E::Solution>, Error<E, S>>
    where
        E: InitAimless<S>,
        ...
        S: Keyed<Key = KeyOf<NodeOf<E>>>,
    {
        let mut best_solution =
            Minimum::new(|u: &E::Solution, v: &E::Solution| u.cost().cmp(&v.cost()));
            ...
            if !already_have_solution {
                ...
                let tree_arc = {
                    self.trees
                        ...
                        .or_insert_with(|| {
                            new_tree_entry(Tree::new(start.clone(), self.expander.clone()))
                        })
                        .clone()
                };
                ...
                if let Some(reached_goal) = tree.closed().get(to) {
                    ...
                    let solution = self
                        .expander
                        .make_solution(reached_goal)
                        .map_err(ErrorImpl::Solve)?;
                    self.solutions
                        .lock()
                        .map_err(|_| ErrorImpl::PoisenedMutex)?
                        .borrow_mut()
                        .insert((start_key.clone(), to.clone()), Some(solution.clone()));
                        ...
                        for node in tree.grow() {
                            ...
                            if node.partial_key() == Some(to) {
                                ...
                                self.solutions
                                    ...
                                    .map_err(|_| ErrorImpl::PoisenedMutex)?
                                    .borrow_mut()
                                    .insert((start_key.clone(), to.clone()), Some(solution));
                                found_solution = true;
                            }
                        }
    ...
    ReverseNodeOf<E>: Weighted + Keyed,
{
        ...
        ReverseNodeOf<E>: Weighted<Cost = CostOf<E>>,
    {
        ...
        for forward in self.expander.aimless_start(from) {
            ...
            for reverse in self.reverser.aimless_start(to) {
                ...
                if let Some((f, r)) = self.grow_best_connection(
                    forward.clone(),
                    key_f.clone(),
                    reverse.clone(),
                    key_r.clone(),
                    ...
                    let solution = self
                        .expander
                        .make_bidirectional_solution(&f, &r)
                        .map_err(BidirExpansionError::Solve)?;
                    best_solution.consider(&solution);
                    self.solutions
                        .lock()
                        .map_err(|_| BidirInitError::PoisonedMutex)?
                        .borrow_mut()
                        .insert((key_f.clone(), key_r.clone()), Some(solution));
                } else {
                    self.solutions
                        .lock()
                        .map_err(|_| BidirInitError::PoisonedMutex)?
                        .borrow_mut()
                        .insert((key_f.clone(), key_r.clone()), None);
                }
            }
        }
        ...
        ReverseNodeOf<E>: Weighted<Cost = CostOf<E>>,
    {
        let mut best_connection = Minimum::new(
            |u: &(Arc<E::Node>, Arc<<E::Reverse as Expander>::Node>),
             v: &(Arc<E::Node>, Arc<<E::Reverse as Expander>::Node>)| {
        ...
        let forward_tree_guard = forward_tree_arc
            .lock()
            .map_err(|_| BidirExpansionError::PoisonedMutex)?;
        ...
        let reverse_tree_guard = reverse_tree_arc
            .lock()
            .map_err(|_| BidirExpansionError::PoisonedMutex)?;
...
mod tests {
    ...
    use crate::{
        ...
        motion::r2::{graph_search::make_default_expander, timed_position::LineFollow, Point},
    };
    use std::sync::Arc;
    ...
    fn test_simple_garden() {
        let garden = Garden::new(Arc::new(make_default_expander(
